// JUST FOR TESTING PURPOSES, DO NOT ACTUALLY DATA FILE SHADERS.
...
vec3 srgb2lin_fast(vec3 c) {
    return c * c;
}
vec3 lin2srgb_fast(vec3 c) {
    return sqrt(c);
}

vec4 unpack_noise(vec4 noise) {
    return mix(vec4(0.5, 0.5, 0.5, 0.5), mix(vec4(-0.5), vec4(1.5), noise), dithering_amount);
} // converts [0.0,1.0] to [-0.5,1.5], which is ideal for dithering

#ifdef DITHER
vec3 dither(vec3 c, float noise, float ratio) {
    return c + noise / ratio;
}
vec3 dither_srgb(vec3 c, float noise, float ratio) {
    return srgb2lin_fast(dither(lin2srgb_fast(c), noise, ratio));
}
#else
vec3 dither(vec3 c, float noise, float ratio) {
    return c;
...
void main()
{
    ...
    float conga_heretical_strength = 1.0;
    tex_coord -= vec2(0.5);
    float conga_theta = atan(tex_coord.y, tex_coord.x);
    float conga_magnitude = length(tex_coord);
    float conga_max_magnitude = pow(1.0 + pow(min(abs(tex_coord.x / tex_coord.y), abs(tex_coord.y / tex_coord.x)), 2.0), 0.5) / 2.0;
    float conga_radial_magnitude = conga_magnitude / conga_max_magnitude;
    conga_radial_magnitude = pow(conga_radial_magnitude, 1.0 + pow(conga_radial_magnitude, 0.5));
    conga_magnitude = conga_radial_magnitude * conga_max_magnitude;
    tex_coord = conga_magnitude * vec2(cos(conga_theta), sin(conga_theta));
    tex_coord += vec2(0.5);
    float conga_heretical_redness = mod(sin(time) * abs(mod(tex_coord.x + tex_coord.y, 0.2 / conga_heretical_strength) - 0.1 / conga_heretical_strength) +
                cos(time) * abs(mod(tex_coord.x - tex_coord.y, 0.2 / conga_heretical_strength) - 0.1 / conga_heretical_strength) + time, 0.2 * conga_heretical_strength);
    conga_heretical_redness *= pow(conga_radial_magnitude, 2.0) * conga_heretical_strength * 2.0;

    gl_FragColor.rg = tex_coord;
    gl_FragColor.a = 1.0;
    //return;
    ...
    gl_FragColor.r += conga_heretical_redness;
    gl_FragColor.a = 1.0;
    gl_FragColor.a = ((5.0 * conga_heretical_strength) * (1.0 - pow(conga_radial_magnitude, 0.5)) + gl_FragColor.a) / (1.0 + 5.0 * conga_heretical_strength); //((conga_heretical_strength) * (1.0 - pow(conga_radial_magnitude, 0.5)) + gl_FragColor.a) / (conga_heretical_strength + 1.0);
}
